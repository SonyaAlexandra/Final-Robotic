...
void setup() {
  ...
  pid.SetOutputLimits(0, 255); // Adjust motor speed range
}
...
void turnRight() {
  ...
  analogWrite(ENA, 150);
  analogWrite(ENB, 150);
  delay(500); // Reduced delay for smoother turning
  stopCar();
}
...
void loop() {
  ...
  while (true) {
    ...
    input = distance;
    pid.Compute();
    moveCar(output); // Adjust speed dynamically
